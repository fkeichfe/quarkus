...
to your `SessionFactory` by simply defining a CDI bean:
...
----
<1> Assign a CDI scope to the interceptor.
`@ApplicationScoped` will create one instance of the interceptor shared among all sessions;
`@Dependent` will create one dedicated instance of the interceptor for each session.
...
[TIP]
====
By default (when no CDI qualifier is applied), the interceptor is assigned to the default persistence unit.

Use `@io.quarkus.hibernate.orm.PersistenceUnit(name = ...)` to assign the interceptor to a named persistence unit.
====
